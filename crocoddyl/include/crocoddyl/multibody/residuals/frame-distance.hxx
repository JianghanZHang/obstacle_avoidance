...
ResidualModelFrameDistanceTpl<Scalar>::ResidualModelFrameDistanceTpl(
...
template <typename Scalar>
void ResidualModelFrameDistanceTpl<Scalar>::calc(
  ...
  data->r.setConstant((d->pinocchio->oMf[id_].translation() - xref_).squaredNorm());
}

template <typename Scalar>
void ResidualModelFrameDistanceTpl<Scalar>::calcDiff(
    const boost::shared_ptr<ResidualDataAbstract>& data,
    const Eigen::Ref<const VectorXs>&, const Eigen::Ref<const VectorXs>&) {
  Data* d = static_cast<Data*>(data.get());
  ...
  d->Rx.leftCols(nv).noalias() = (d->pinocchio->oMf[id_].translation() - xref_).normalized().transpose()* 
      d->pinocchio->oMf[id_].rotation() * d->fJf.template topRows<3>();
}

template <typename Scalar>
boost::shared_ptr<ResidualDataAbstractTpl<Scalar> >
ResidualModelFrameDistanceTpl<Scalar>::createData(
...
template <typename Scalar>
void ResidualModelFrameDistanceTpl<Scalar>::print(std::ostream& os) const {
  const Eigen::IOFormat fmt(2, Eigen::DontAlignCols, ", ", ";\n", "", "", "[",
                            "]");
  os << "ResidualModelFrameDistance {frame=" << pin_model_->frames[id_].name
     << ", tref=" << xref_.transpose().format(fmt) << "}";
}

template <typename Scalar>
pinocchio::FrameIndex ResidualModelFrameDistanceTpl<Scalar>::get_id() const {
  return id_;
}

template <typename Scalar>
const typename MathBaseTpl<Scalar>::Vector3s&
ResidualModelFrameDistanceTpl<Scalar>::get_reference() const {
  return xref_;
}

template <typename Scalar>
void ResidualModelFrameDistanceTpl<Scalar>::set_id(
    const pinocchio::FrameIndex id) {
  id_ = id;
}

template <typename Scalar>
void ResidualModelFrameDistanceTpl<Scalar>::set_reference(
