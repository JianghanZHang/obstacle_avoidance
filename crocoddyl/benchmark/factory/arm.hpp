...
void build_arm_action_models(
  ...
  typedef typename crocoddyl::ResidualModelFrameDistanceTpl<Scalar>
      ResidualModelFrameSphereDistance;
...
boost::shared_ptr<crocoddyl::ActivationModelAbstract> obstacle_avoidance_activation =
      boost::make_shared<crocoddyl::ActivationModelExpBarrier>(3, 0.1, 10);

  boost::shared_ptr<CostModelAbstract> obstacleAvodianceCost =
      boost::make_shared<CostModelResidual>(
            state,
            obstacle_avoidance_activation,
            boost::make_shared<ResidualModelFrameTranslation>(
                     state, model.getFrameId("gripper_left_joint"),
                         Vector3s(Scalar(0), Scalar(0), Scalar(.4))));
  ...
  runningCostModel->addCost("obstacleAvoidance", obstacleAvodianceCost, Scalar(1));
  terminalCostModel->addCost("gripperPose", goalTrackingCost, Scalar(1));
  terminalCostModel->addCost("obstacleAvoidance", obstacleAvodianceCost, Scalar(1));
